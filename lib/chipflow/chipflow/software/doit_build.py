...
BIOS_START = f"{BUILD_DIR}/generated/start.S"
...
def task_build_bios_elf():
    def get_build_cmd():
        ...
        return f"{RISCVGCC} {CFLAGS} -o {BUILD_DIR}/bios.elf {BIOS_START} {sources_src}"
    ...
    return {
        ...
        "targets": [f"{BUILD_DIR}/bios.elf"],
        "verbosity": 2
    }


def task_build_bios_bin():
    return {
        "actions": [f"{RISCVOBJCOPY} -O binary {BUILD_DIR}/bios.elf {BUILD_DIR}/bios.bin"],
        "file_dep": [f"{BUILD_DIR}/bios.elf"],
        "targets": [f"{BUILD_DIR}/bios.bin"],
    }
